...
class FirebaseService {
  ...
  async registerUser(userData: {
    ...
    await this.linkGuestAccountsOnRegistration(
      userData.uid,
      normalizedEmail,
   ...
   */
  private async linkGuestAccountsOnRegistration(
    userId: string,
    email: string,
    displayName: string
  ): Promise<void> {
    try {
      ...
      for (const diaryDoc of diariesSnap.docs) {
        ...
        if (guestEntry) {
          ...
          console.log(
            "📧 Created invitation for newly registered user:",
            email
          );
        }
      }
